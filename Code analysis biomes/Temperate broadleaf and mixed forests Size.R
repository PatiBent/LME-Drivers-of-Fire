...
saveRDS(LMfinal, "LMBiome4Size.rds")
...
M.glsw3 <- gls(f, data =Data,
               weights = vf13)

#compare AIC
AIC(GLM,M.gls1,M.gls2,M.gls3,M.gls4,M.gls5,M.gls6,M.gls7, M.gls8, M.gls9, M.gls10, M.glsw, M.glsw2, M.glsw3)

#check homogeneity of variance again
summary(M.glsw)
plot(M.glsw, main="M.glsw")

#End find a good variance structure; homogenity looks better but still not great 
...
#drop IDL_LayerNo_IDL:I(LC_Shrub^2) & IDL_LayerNo_IDL:I(DTR^2) & IDL_LayerNo_IDL:DistUrbPop & IDL_LayerNo_IDL:LC_Shrub & IDL_LayerNo_IDL:LC_Tree
ffix1<-formula(Size~IDL_Layer+LC_Tree+LC_Shrub+IDL_Layer*LC_Herbs
               +IDL_Layer*DTR+IDL_Layer*TMax+IDL_Layer*fPAR+IDL_Layer*LC_Crop+IDL_Layer*Precipitation+IDL_Layer*NrWetDays +IDL_Layer*RoadDensity_sqrt+IDL_Layer*PopDensity_log+DistUrbPop
               +IDL_Layer*I(TMax^2)+I(DTR^2)+IDL_Layer*I(fPAR^2)+ IDL_Layer*I(LC_Tree^2) +I(LC_Shrub^2)+IDL_Layer*I(LC_Crop^2)+IDL_Layer*I(LC_Herbs^2)+IDL_Layer*I(Precipitation^2) +IDL_Layer*I(NrWetDays^2)
               +IDL_Layer*I(RoadDensity_sqrt^2)+IDL_Layer*I(PopDensity_log^2)+IDL_Layer*I(DistUrbPop^2)
)
...
ffix2<-formula(Size~IDL_Layer+LC_Tree+LC_Shrub+IDL_Layer*LC_Herbs
               +IDL_Layer*DTR+IDL_Layer*TMax+IDL_Layer*fPAR+IDL_Layer*LC_Crop+IDL_Layer*Precipitation+IDL_Layer*NrWetDays +IDL_Layer*RoadDensity_sqrt+IDL_Layer*PopDensity_log+DistUrbPop
               +IDL_Layer*I(TMax^2)+I(DTR^2)+IDL_Layer*I(fPAR^2)+ IDL_Layer*I(LC_Tree^2) +I(LC_Shrub^2)+IDL_Layer*I(LC_Crop^2)+IDL_Layer*I(LC_Herbs^2)+IDL_Layer*I(Precipitation^2) +IDL_Layer*I(NrWetDays^2)
               +IDL_Layer*I(RoadDensity_sqrt^2)+IDL_Layer*I(PopDensity_log^2)
)
...
#-> Homogenity and normality of the residuals looks ok and good respectively. Model with the best AIC as well
...
#set path
setwd("~/GitHub/GLM-Drivers-of-Fire/Output/Model Results")
saveRDS(M.glswc, "lmeBiome4Size.rds")
